...
from getFrame2cv import getFrame
from getFrame2cv.MvCameraControl_class import *
import numpy as np
...
class mytkinter(tk.Tk):
    ...
    def setupUi(self):
        ...
        self.cam,self.stOutFrame,self.data_buf=getFrame.start_cam(nConnectionNum = int(self.var6.get()))
    ...
    def update_view(self):
        ...
        if ret == 0:
            ...
            % (self.stOutFrame.stFrameInfo.nWidth, self.stOutFrame.stFrameInfo.nHeight, self.stOutFrame.stFrameInfo.enPixelType, self.stOutFrame.stFrameInfo.nFrameNum,self.stOutFrame.stFrameInfo.nFrameLen))

            buf_cache = (c_ubyte * self.stOutFrame.stFrameInfo.nFrameLen)()
                # 图像数据拷贝
            memmove(byref(buf_cache), self.stOutFrame.pBufAddr, self.stOutFrame.stFrameInfo.nFrameLen)
            # print(self.stOutFrame.stFrameInfo.enPixelType)

            g_numArray = self.Mono_numpy(buf_cache,self.stOutFrame.stFrameInfo.nWidth, self.stOutFrame.stFrameInfo.nHeight)
            g_numArray = g_numArray.reshape(self.stOutFrame.stFrameInfo.nHeight, self.stOutFrame.stFrameInfo.nWidth, 1)
            frame=cv2.cvtColor(g_numArray,cv2.COLOR_BAYER_RG2BGR)
            print(frame.shape)
        ...
        if ret==0 and self.ifstart:
            ...
            boxes_list=list(boxes.xywh)

            HSs=[]
            for i in range(len(boxes_list)):
                w=list(boxes_list[i])[2]
                h=list(boxes_list[i])[3]
                D=(w+h)/2
                print(float(D))
                HSs.append(round(float(D/5.9),1))

            HSs.sort()
            print([22,29,35,44,51,64,76,92,105,114])
            print(HSs)
            ...
            imgtk = ImageTk.PhotoImage(image=Image.fromarray(res_plotted))
    ...
    def __del__(self):
        getFrame.exit_cam(self.cam,self.data_buf)
