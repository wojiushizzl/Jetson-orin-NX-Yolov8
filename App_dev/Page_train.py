...
def train_project(selected_projects):
    ...
    patience = int(train_grid.number_input('patience', min_value=10,max_value=epochs,  value=epochs,
...
def deploy(selected_projects):
    ...
    if last_train_path is not None:
        st.caption(f"last_train_path :blue[{last_train_path}]")
        best_path = os.path.join(last_train_path, 'weights', 'best.pt')
        st.caption(f"last train weights file path :blue[{best_path}]")
        if st.button('一键部署'):
    ...
    if last_train_path is not None:
        ...
        try:
            ...
            if image_list is not None:
                image_list.sort()
                image_grid=grid([2,2,2,2],vertical_align='bottom')
                for img in image_list:
                    image_grid.image(img, caption=os.path.basename(img), use_column_width=True)
        except:
